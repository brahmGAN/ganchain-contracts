...
describe("GPU", () => {
  ...
  before(async () => {
    [owner, user1, user2, helper, scheduler, provider] = await ethers.getSigners();
    ...
    gpuProxy = await upgrades.deployProxy(
      gpuFactory,
      [nftContract.target, 69, 1, 1, 1, 1, 1, 1, 1, 1, 1, helper.address, scheduler.address],
  ...
  describe("Add user as a validator,queen and as a provider", () => {
    it("Should be able to add user1 as a Validator", async () => {
      await expect(
        await gpuProxy
          .connect(user1)
          .addValidator(validatorSS58Address)
      )
