...
contract Reward is OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Mappings
    mapping (address => uint96) public providerRewards;
    mapping (address => uint40) public lastWithdrawalTime;
    ...
    function initialize(address _GPUAddress, uint96 RewardGPsPerDay) public initializer {
    ...
    function accumulateDailyProviderRewards() public onlyOwner {
        ...
        uint24 providersLength = uint24(providers.length); 
        for (uint24 i = 0; i < providersLength; i++) {
            ...
            uint8 computeMultiplier = getComputeMultiplier(computeUnits);
        ...
        if (totalComputeScore > 0) { // Avoid division by zero
            for (uint24 i = 0; i < providersLength; i++) {
                ...
                uint256 todaysReward = (computeScores[i] * rewardGPsPerDay ) / (totalComputeScore); // In wei
                address providerNFTAddress = GPUInstance.getNFTAddress(providers[i]);
                providerRewards[providerNFTAddress] += uint96(todaysReward);
            }
        }
        lastRewardCalculated = uint40(block.timestamp);
    ...
    function getComputeMultiplier(uint256 computeUnits) public pure returns(uint8) {
        if (computeUnits < 81) {
            return 100;
